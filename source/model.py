...
class LineConv(Module):
    ...
    def forward(self, item_embedding, D, A, session_item, session_len):
        ...
        zeros = zeros.to(item_embedding.device)
        ...
        # Move each tensor in session to CPU and convert to NumPy array
        for tensor in session:
            session_numpy.append(tensor.cpu().numpy())

        # Convert the list of NumPy arrays to a single NumPy array and perform sum operation
        session_emb_lgcn = np.sum(np.array(session_numpy), axis=0) / (self.layers + 1)
...
def forward(model, i, data):
    ...
    session_item = session_item.to('cuda')
    session_len = session_len.to('cuda')
    D_hat = D_hat.to('cuda')
    A_hat = A_hat.to('cuda')
    reversed_sess_item = reversed_sess_item.to('cuda')
    mask = mask.to('cuda')
...
def train_test(model, train_data, test_data, progress_bar=None):
    ...
    if progress_bar:
        progress_bar.reset(total=len(slices))  # Reset progress bar with total number of batches
    ...
    for i in slices: 
        ...
        if progress_bar:
            progress_bar.update(1)  # Update progress bar for each batch
